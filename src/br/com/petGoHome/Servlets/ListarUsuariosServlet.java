		...
		try {
			...
			try {
				if (p.getTipoUsuario() == TipoUsuario.ADMINISTRADOR) {
					...
					int qtdePag = Fachada.getQtdePag();
					...
					pessoas = Fachada.listarTodasPessoasLimitadas(pag);
					
					//LIsta o Numero total de Pessoas do banco
					List<Pessoa> pessoasListaTotal = new ArrayList<>();
					pessoasListaTotal = Fachada.listarTodasPessoas();
					
					int qtLista = pessoasListaTotal.size();
					request.setAttribute("qtLista", qtLista);
					
					
					request.setAttribute("pessoas", pessoas);
					request.setAttribute("listagemUsuario", listagemUsuario);
					request.setAttribute("qtdePages", qtdePag);
					request.getRequestDispatcher(
							"AdminLTE-master/paginas/Listar/listar-usuarios.jsp").forward(request,
									response);
				}else{
