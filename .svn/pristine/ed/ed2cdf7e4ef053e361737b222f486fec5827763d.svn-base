	...
	function validarSenha() {
		...
		if (senha == senha2) {
			...
			if (senha == "" && senha2 == "") {
				$('#msgSenhaErrada')
						.html(
								"<div style='width:50%' align='center' class='alert alert-warning'><button type='button' class='close' data-dismiss='alert'>x</button><img src='img/exclamacao.png' /><h5>É obrigatório preencher as senhas.</h5></div>")
						.prop('disabled', false);
				...
				setTimeout(function() {
					jQuery('#msgSenhaErrada').hide("slow");
				}, 5000);
			} else {
				//alert("SENHAS IGUAIS");
			...
			$('#msgSenhaErrada')
					.html(
							"<div style='width:50%' align='center' class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>x</button><img src='img/exclamacao.png' /><h5>Senhas com valores diferentes. Redigite, Por favor.</h5></div>")
					.prop('disabled', false);
			setTimeout(function() {
				jQuery('#msgSenhaErrada').show();
			});
				...
				<div class="tab-content no-padding">
					...
					<form id="contact-form" action="Alterar-sua-Senha.pets"
						...
						<div class="form-group">
							<label class="senha">Senha</label> <input class="form-control" type="password" id="senha"
								name="senha" required="required" autocomplete="off"
								class="marcar" placeholder="Senha:"> 
						</div>
						<div class="form-group">
							<label class="senha">Confirmar senha</label> <input class="form-control" type="password" id="senha2"
								name="senha2" required="required" autocomplete="off"
								onBlur="validarSenha()" class="marcar"
						...
						<div class="form-group">
							<button type="submit" class="btn btn-large btn-warning"
								id="Enviar">
								<i class="icon-edit icon-white"></i> Alterar
							</button>
						</div>
					</form>
