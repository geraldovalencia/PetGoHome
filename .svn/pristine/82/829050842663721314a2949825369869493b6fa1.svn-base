...
<section class="content-header">
	<h1>
		Alterar Perfil <small>de Gato</small>
	</h1>
	<ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-home"></i> Home</a></li>
		<li class="active">Alterar Gato</li>
	</ol>
</section>
...
<section class="content">
	...
	<div class="row">
		...
		<section class="col-lg-7 connectedSortable ui-sortable">
			...
			<!-- Alterar Dados de Gato ------------------------------------------------ -->
			<div class="box box-info">
				<div class="box-header ui-sortable-handle" style="cursor: move;">
					<i class="fa fa-pencil-square-o"></i>
					<h3 class="box-title">Alterar a dados de ${cat != null ? cat.nome : 'seu Pet.'}
					<!-- tools box -->
				...
				<div class="box-body">
					<form id="contact-form" action="Editar-Gato.pets" method="post" name="chavez">
						<input type='hidden' name='idGatoEditado'
							value='${cat != null ? cat.id : ""}' />

						<div class="form-group">
							<label>Nome</label> <input value="${cat != null ? cat.nome : ''}"
							...
							<label>Raça</label> <select name='raca' required="required"
								...
								<c:forEach var="raca" items="${racas}">
									<option value="${raca.id}"
										${raca.id == cat.racaGato.id ? 'selected="selected"' : ''}>${raca.nomeRacaGato}</option>
						...
						<div class="form-group">
							<label>Cor</label> <input class="form-control"
								value="${cat != null ? cat.cor : ''}" type="text"
						...
						<div class="form-group">
							<label>Sexo</label> <select class="form-control" name="sexo"
								...
								<option value="Masculino"
									${cat.sexo == 'MASCULINO' ? 'selected="selected"' : ''}>Sexo:
									Masculino</option>
								<option value="Feminino"
									${cat.sexo == 'FEMININO' ? 'selected="selected"' : ''}>Sexo:
									Feminino</option>
							</select>
						</div>
						...
						<div>
							...
							<textarea class="form-control" name="descricao"
								placeholder="Descrição: breve do pet (tamanho, cor) e mais informações relevantes, como observações médicas: ">${cat != null ? cat.descricao : ''}</textarea>
							...
							<c:choose>
								<c:when test="${pessoaLogado != null}">
									<div class="form-group">
										...
										<c:if test="${cat.nomeImagem != null}">
											<a id="example1" href="images/${cat.nomeImagem}"><img
												style="width: 80%; height: 100%"
												class="img-rounded form-control animated fadeIn"
												alt="Clique para ampliar" src="images/${cat.nomeImagem}"></a>
										</c:if>
										<c:if
											test="${cat.nomeImagem == null || cat.nomeImagem == '' }">
										...
										<br> <a
											href="Alterar-Foto-do-seu-Gato.pets?idGato=${cat.id}&idPessoa=${cat.pessoa.id}">
										...
										<c:if test="${cat.tagNumero != null}">
											...
											<h3 style="margin-left: 10%;">
												Sua Tag é: <span class="txt-verde">${cat.tagNumero }</span>
											</h3>
										</c:if>
										<c:if test="${cat.tagNumero == null}">
											...
											<h3>
												Cadastre uma tag: <a
													href="Vincular-Tag-dos.pets?idcat=${cat.id}&idPessoa=${cat.pessoa.id}"
